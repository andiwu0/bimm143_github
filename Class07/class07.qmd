...
author: "Andrew Duong (PID: A128129684)"
format: pdf
---
...
Let's make up some input data for cluttering where we know what the natural "clusters" are.
...
The main funciton in "base R" for K-means clustering is called `kmeans()`:
...
The main function for Hierarchical Clusterin is called `hclust()`. Unlike `kmeans()` (which does all the work for you) you can't just pass `hclust()` our raw input data. It needs a  "distance matrix" like the one returned from the `dist()` function.
