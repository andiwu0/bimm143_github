...
> **N.B.** We never run an install.packages in a code chunk otherwise we will re-install needlessly every time we redner our document.
...
3. The **geom_** or geometry i.e. the plot type such as points, line etc.
...
Read some data on the effects of GLP-1 inhbitor (drug) on gene expression values: 
