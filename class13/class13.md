...
Today we will perform an RNASeq analysis on the effects of dexamthasone
(hereafter “dex”), a common steroid, on airway smooth muslce (ASM) cell
lines.
...
> Q. Add a new column `log2fc` for log2 fold change of treated/contorl
...
about them and they should by excluded from further consideration. These
...
The arr.ind argument calls for the row and column indices. We take the
first column because the first column shows which genes have zeros.
unique() is used because a gene could have zeros as the control and
treated samples.
...
A useful summary figure of our results is often cakked a volcano plot.
...
We will first need to install these bioconductor with
...
No we know the gene names (gene symbols) and their entrez IDs we can
...
To run our pathway analysis, we wil use the **gage()** function. It
...
```
    ...
    Info: Working in directory C:/Users/andre/OneDrive/Desktop/BIMM143/bimm143_github/class13
